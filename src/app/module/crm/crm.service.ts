...
const chooseAgent = async (organizationId: string, lead: { locationPreference?: string }, preferredPropertyAgent?: string) => {
  ...
  if (config.assignment.mode === 'manual') return { agentId: undefined, strategy: 'manual' as const, reason: 'Manual assignment configured' }
  let agents = await activeEligibleAgents(organizationId, config.assignment.eligibleAgentIds || [])
  if (!agents.length) return { agentId: undefined, strategy: config.assignment.mode as any, reason: 'No eligible active agents' }

  if (config.assignment.mode === 'territory' && lead.locationPreference) {
    const location = lead.locationPreference.toLowerCase()
    const rules = [...(config.assignment.territoryRules || [])].sort((a: any, b: any) => b.priority - a.priority)
  ...
  if (config.assignment.mode === 'workload' || config.assignment.mode === 'territory') {
    ...
    const cap = config.assignment.workloadCap || 100
    ...
    return { agentId: agents[0]?._id?.toString(), strategy: config.assignment.mode as 'territory' | 'workload', reason: agents.length ? 'Lowest eligible open-lead workload' : 'All eligible agents are at workload cap' }
  }

  const cursor = Math.max(0, config.assignment.roundRobinCursor || 0)
