...
const stage = z.object({ key: z.string().trim().min(1).max(40), label: z.string().trim().min(1).max(80), color: z.string().trim().max(30).optional(), order: z.number().int().min(0), terminal: z.boolean().optional(), won: z.boolean().optional(), lost: z.boolean().optional() })
