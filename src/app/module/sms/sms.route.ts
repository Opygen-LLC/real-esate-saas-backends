...
import { outboundSmsRateLimiter, webhookRateLimiter } from '../../middlewares/rateLimiter'
...
router.post('/delivery-receipt', webhookRateLimiter, (req, _res, next) => { const secret = req.headers['x-sms-webhook-secret']; if (!config.sms.webhook_secret || secret !== config.sms.webhook_secret) return next(new ApiError(401, 'Invalid SMS webhook signature')); next() }, validateRequest(receiptSchema), SmsController.receipt)
router.post('/send', authMiddlewares.requirePermission('messaging.manage'), outboundSmsRateLimiter, validateRequest(sendSchema), SmsController.send)
