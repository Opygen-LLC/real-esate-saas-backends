...
const send = catchAsync(async (req: Request, res: Response) => { const organizationId = requireTenant(req); await EntitlementService.assertFeature(organizationId, 'smsAutomation'); const data = await SmsService.send(organizationId, { ...req.body, sentBy: req.user?._id }); sendResponse(res, { statusCode: 202, success: true, message: 'SMS accepted', data }) })
