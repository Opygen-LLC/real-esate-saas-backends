...
const getTlsStatus = async (input: DomainProviderInput): Promise<DomainTlsResult> => {
  ...
  const active = registered && sslActive(apex) && sslActive(www)
  const statuses = {
    apex: { hostname: apex?.hostname || input.domain, status: apex?.status || 'missing', sslStatus: apex?.ssl?.status || 'missing', errors: sslErrors(apex) },
    www: { hostname: www?.hostname || `www.${input.domain}`, status: www?.status || 'missing', sslStatus: www?.ssl?.status || 'missing', errors: sslErrors(www) },
  }
  const failed = !registered || [apex, www].some((item) => SSL_FAILURE_STATUSES.has(String(item?.ssl?.status || '')))
  ...
  const message = !registered
    ...
    : failed
      ? 'Cloudflare certificate issuance failed or timed out; review the SSL validation records and validation errors'
      : undefined
  return {
    status,
    providerMetadata: metadataFor([apex, www]),
    diagnostics: [diagnostic('tls_certificate', 'Cloudflare SSL for SaaS certificate', active, {
      expected: 'Cloudflare ssl.status=active for apex and www',
      observed: statuses,
...
export const CloudflareDomainProvider: DomainProvider = {
  ...
  async verifyRouting(input): Promise<DomainRoutingResult> {
    ...
    const registered = Boolean(apex && www)
    const providerVerified = hostnameReady(apex) && hostnameReady(www)
    ...
    return {
      ...
      providerMetadata: metadataFor([apex, www]),
      diagnostics: [
        diagnostic('apex_a', 'Apex Cloudflare for SaaS routing', apexOk, {
          expected: { cnameTarget: normalizeTarget(config.domains.cloudflare_saas_cname_target), flatteningAllowed: true },
          observed: apexDns,
          state: apexOk ? 'pass' : 'pending',
          ...(!apexOk ? { message: `Point the apex/root hostname to ${normalizeTarget(config.domains.cloudflare_saas_cname_target)} using CNAME, ALIAS/ANAME, or supported CNAME flattening` } : {}),
        }),
        ...
        diagnostic('hosting_registration', 'Cloudflare custom-hostname validation', registered && providerVerified, {
          expected: 'Apex and www registered with Cloudflare and hostname status active',
          ...
          ...(!registered
            ...
            : !providerVerified
              ? { message: 'Cloudflare hostname ownership validation is pending. Add every Cloudflare hostname-validation record shown in Required DNS records.' }
              : {}),
        }),
      ],
    }
  ...
  async verifyPublicRouting(input): Promise<DomainPublicRoutingResult> {
    ...
    return {
      ...
      diagnostics: [diagnostic('public_routing', 'Cloudflare public routing', active, {
        ...
        ...(!active ? { message: 'Cloudflare is not yet serving the Opygen runtime for both hostnames; DNS propagation, Worker routing, or certificate deployment may still be pending' } : {}),
      })],
    }
  },
