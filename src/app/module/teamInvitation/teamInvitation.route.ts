...
import { outboundEmailRateLimiter, publicLeadRateLimiter } from '../../middlewares/rateLimiter'
...
router.post('/accept', publicLeadRateLimiter, validateRequest(z.object({ body: z.object({ token: z.string().min(20).max(200), password: strongPasswordSchema }).strict() })), TeamInvitationController.accept)
