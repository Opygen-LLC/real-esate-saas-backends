...
export const WebsiteBuilderValidation = {
  ...
  presignAssetSchema: z.object({ body: z.object({ filename: z.string().trim().min(1).max(255).refine((value) => !/[\/\\\u0000]/.test(value), 'Invalid filename'), mimeType: z.enum(['image/jpeg','image/png','image/webp','image/avif','font/woff2']), size: z.number().int().positive() }).strict() }),
