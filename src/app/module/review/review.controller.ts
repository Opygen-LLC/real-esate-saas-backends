...
const submit = catchAsync(async (req: Request, res: Response) => {
  const review = await ReviewService.submit(req.body)
  const submission = await WebsiteSubmissionService.captureReview(review)
  sendResponse(res, {
    statusCode: httpStatus.CREATED,
    success: true,
    message: 'Thank you. Your review was submitted for approval.',
    data: WebsiteSubmissionService.withPublicReceipt(review, submission),
  })
})
