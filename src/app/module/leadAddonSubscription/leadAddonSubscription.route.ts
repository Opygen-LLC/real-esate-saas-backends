...
router.get('/admin/subscriptions', adminNetworkRateLimiter, authMiddlewares.authSuperAdmin, adminOperationRateLimiter, validateRequest(z.object({ query: z.object({ page: z.coerce.number().int().min(1).optional(), limit: z.coerce.number().int().min(1).max(100).optional(), status: z.enum(['all', 'pending_payment', 'active', 'cancel_at_period_end', 'cancelled', 'expired', 'payment_failed', 'rejected']).optional(), organizationId: z.string().trim().min(1).optional() }) })), LeadAddonSubscriptionController.listAdmin)
router.patch('/admin/subscriptions/:id/decision', adminNetworkRateLimiter, authMiddlewares.authSuperAdmin, adminOperationRateLimiter, validateRequest(LeadAddonSubscriptionValidation.decision), LeadAddonSubscriptionController.decide)
