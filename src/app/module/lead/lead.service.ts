...
const getAllLeads=async(filters:ILeadFilter,paginationOptions:IPaginationOptions):Promise<IGenericResponse<ILead[]>>=>{const{searchTerm,organizationId,leadStatus,source,assignedAgent,propertyType,minBudget,maxBudget,sla,minScore}=filters;const conditions:any[]=[];if(organizationId)conditions.push({organizationId});if(searchTerm)conditions.push({$or:['name','email','phone','locationPreference','attribution.utmCampaign'].map(field=>({[field]:{$regex:searchTerm,$options:'i'}}))});if(leadStatus)conditions.push({leadStatus});if(source)conditions.push({source});if(assignedAgent)conditions.push({assignedAgent});if(propertyType)conditions.push({propertyType});if(minBudget!==undefined&&minBudget!=='')conditions.push({budgetMax:{$gte:Number(minBudget)}});if(maxBudget!==undefined&&maxBudget!=='')conditions.push({budgetMin:{$lte:Number(maxBudget)}});if(minScore!==undefined&&minScore!=='')conditions.push({leadScore:{$gte:Number(minScore)}});if(sla==='breached')conditions.push({firstResponseAt:{$exists:false},responseDueAt:{$lt:new Date()}});if(sla==='due')conditions.push({firstResponseAt:{$exists:false},responseDueAt:{$gte:new Date()}});const where=conditions.length?{$and:conditions}:{};const{page,limit,skip,sortBy,sortOrder}=paginationHelper.calculatePagination(paginationOptions);const [result,total]=await Promise.all([Lead.find(where).populate('assignedAgent','name email phoneNumber profileImgURL').populate('propertyInterest','title price images city').populate('contactId','name email phone company').sort({[sortBy]:sortOrder}).skip(skip).limit(limit),Lead.countDocuments(where)]);return{meta:{page,limit,total},data:result}}
const getLeadById=async(organizationId:string,id:string)=>{const result=await Lead.findOne({_id:id,organizationId}).populate('assignedAgent','name email phoneNumber profileImgURL licenseNumber').populate('propertyInterest','title price images city propertyType bedrooms bathrooms').populate('contactId','name email phone address company notes tags');if(!result)throw new ApiError(404,'Lead not found');return result}
const updateLead=async(organizationId:string,id:string,payload:Partial<ILead>,actorId?:string)=>{const current:any=await Lead.findOne({_id:id,organizationId});if(!current)throw new ApiError(404,'Lead not found');if(payload.phone){payload.phone=normalizePhone(payload.phone);payload.normalizedPhone=payload.phone}if(payload.email!==undefined)payload.normalizedEmail=normalizeOptionalEmail(payload.email);const scored=scoreLead({...current.toObject(),...payload});payload.leadScore=scored.score;payload.scoreReasons=scored.reasons;const result=await Lead.findOneAndUpdate({_id:id,organizationId},payload,{new:true,runValidators:true}).populate('assignedAgent','name email phoneNumber profileImgURL').populate('propertyInterest','title price images city');await DomainEventService.emit({organizationId,aggregateType:'lead',aggregateId:id,eventType:'lead.updated',leadId:id,actorId,payload:{summary:'Lead profile fields updated',fields:Object.keys(payload)}});return result}
...
const deleteLead=async(organizationId:string,id:string)=>{const result=await Lead.findOneAndDelete({_id:id,organizationId});if(!result)throw new ApiError(404,'Lead not found');return result}
