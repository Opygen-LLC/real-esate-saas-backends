...
router.get('/all', authMiddlewares.authSuperAdmin, validateRequest(z.object({ query: z.object({ page: z.coerce.number().int().min(1).max(100000).optional(), limit: z.coerce.number().int().min(1).max(100).optional(), status: status.optional(), priority: priority.optional(), ownerId: z.union([mongoId, z.literal('unassigned')]).optional(), sla: z.literal('breached').optional(), search: z.string().trim().max(160).optional() }).strict() })), SupportController.getAllTicketsSuperAdmin)
...
router.post('/:id/attachments/:attachmentId/complete', authMiddlewares.auth(), validateRequest(z.object({ params: attachmentParams, body: z.object({}).strict().optional() })), SupportController.completeAttachmentUpload)
