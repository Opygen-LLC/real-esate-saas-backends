...
const hardDeleteTenant = catchAsync(async (req: Request, res: Response) => sendResponse(res, { statusCode: 200, success: true, message: 'Organization and all associated database records were permanently deleted', data: await PlatformAdminService.hardDeleteTenant(req.params.organizationId, req.body, { id: req.user!._id!, reason: req.body.reason, requestId: req.requestId, ip: req.ip, userAgent: req.get('user-agent') || '' }) }))
