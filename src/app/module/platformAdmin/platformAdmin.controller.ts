...
const editSubscriptionDates = catchAsync(async (req: Request, res: Response) => sendResponse(res, { statusCode: 200, success: true, message: 'Paid subscription dates updated', data: await PlatformAdminService.editSubscriptionDates(req.params.paymentNumber, { ...req.body, paidAt: new Date(req.body.paidAt), periodStart: new Date(req.body.periodStart), periodEnd: new Date(req.body.periodEnd) }, { id: req.user!._id!, requestId: req.requestId, ip: req.ip }) }))
...
export const PlatformAdminController = { tenantDetails, tenantHealth, suspendTenant, reactivateTenant, updateTenantProfile, updateTenantOwner, archiveTenant, restoreArchivedTenant, tenantDeletionPreview, scheduleTenantDeletion, subscriptionRequests, paymentLedger, benefitPeriodHistory, tenantLeadEntitlement, adjustTenantRenewalStreak, recordManualPayment, decideManualPayment, revenue, audit, subscriptionSummary, changeTenantSubscription, manageTenantTrial, editSubscriptionDates, applyTenantAdminPlanOverride, scheduleTenantAdminDowngrade, cancelTenantScheduledChange, setTenantCancellation, requestTenantRecurringAddon, getTenantEntitlementOverrides, setTenantEntitlementOverride, revokeTenantEntitlementOverride, platformSearch, platformNotifications, startImpersonation, currentImpersonation, endImpersonation }
