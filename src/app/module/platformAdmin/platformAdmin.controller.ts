...
const hardDeleteTenant = catchAsync(async (req: Request, res: Response) => sendResponse(res, { statusCode: 200, success: true, message: 'Organization and all associated data were permanently deleted.', data: await PlatformAdminService.hardDeleteTenant(req.params.organizationId, req.body, { id: req.user!._id!, reason: 'Super Admin permanent organization deletion', requestId: req.requestId, ip: req.ip, userAgent: req.get('user-agent') || '' }) }))
