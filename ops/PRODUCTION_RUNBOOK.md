...
- Customer domains may remain registered and DNS-hosted at GoDaddy, Namecheap, Hostinger, Cloudflare, or another provider. Do **not** ask customers to transfer the domain or change nameservers. Register both apex and `www` on the Opygen Vercel project, then show the Vercel-recommended A/CNAME records plus any Vercel ownership-verification record.
- For Cloudflare-managed DNS, keep the Vercel routing records in **DNS only** mode while provisioning. A proxied/orange-cloud record can hide the expected CNAME and interfere with validation.
- Existing records created while `DOMAIN_PROVIDER=generic` self-heal after the provider switch: the lifecycle registers the hostname on Vercel before checking DNS and replaces stale generic routing instructions with fresh Vercel records. A successful manual/worker verification also resolves superseded failed domain jobs so health does not stay red forever.
